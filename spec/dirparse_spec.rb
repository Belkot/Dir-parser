...
"
  ...
  context 'in temp dir' do
    ...
    it "valid" do
      Dir.mktmpdir do |dir|
        Dir.chdir dir
        FILES.each { |name| File.new(name, "w+") }
        list = %x(ruby #{@curdir}/dirparse.rb #{dir})
        expect(list).to eq RESULT
      end
    end
  ...
  it "show answer when folder not exist" do
    list = %x(ruby dirparse.rb tempNOfolder)
    expect(list).to eq "No is a directory: tempNOfolder.\n"
  end
